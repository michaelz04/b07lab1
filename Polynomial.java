...
public class Polynomial {
    ...
    public Polynomial add(Polynomial p) {
        ...
        int count = p.coefficients.length;

        for (int i = 0; i < p.coefficients.length; i++) {
            arr[i] = p.coefficients[i];
            arr2[i] = p.exponents[i];
        }
        ...
        for (int i = 0; i < coefficients.length; i++) {
            if (exponents[i] == 0) {
                ...
                for (int j = 0; j < p.coefficients.length; j++) {
                    if (arr2[j] == 0) {
                        arr[j] += coefficients[i];
                        check = false;
                    }
                }
                if (check) {
                    arr2[count] += exponents[i];
                    arr[count] += coefficients[i];
                    count++;
                }
                continue;
            }
        ...
        for (int i = 0; i < len; i++) {
            if (arr2[i] == 0) {
                if (arr[i] != 0) {
                    ...
                    q_arr = new double[q_len - 1];
                    q_arr2 = new int[q_len - 1];
                }
                break;
            }
        }
    ...
    public double evaluate(double x) {
        ...
        for (int i = 0; i < coefficients.length; i++) {
            d += coefficients[i] * Math.pow(x, exponents[i]);
        }
        return d;
    }
